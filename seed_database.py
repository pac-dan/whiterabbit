...
def seed_database():
    ...
    with app.app_context():
        ...
        for video in videos:
            existing = Video.query.filter_by(youtube_id=video.youtube_id).first()
            if not existing:
        ...
        for testimonial in testimonials:
            existing = Testimonial.query.filter_by(client_name=testimonial.client_name).first()
            if not existing:
                db.session.add(testimonial)
                print(f"  Added testimonial: {testimonial.client_name}")
