...
def create_app(config_name=None):
    ...
    # TEMPORARY: Admin creation route for Render free tier
    # DELETE THIS ROUTE AFTER CREATING ADMIN!
    @app.route('/temporary-create-admin-delete-after-use')
    def temporary_create_admin():
        """
        TEMPORARY ROUTE - DELETE AFTER CREATING ADMIN!
        This route allows creating an admin user via web request
        ...
        admin_password = os.getenv('ADMIN_PASSWORD', 'TempAdmin123!')
        ...
        try:
            ...
            if existing:
                return jsonify({
                    'status': 'exists',
                    'message': f'User already exists: {existing.email}',
                    'is_admin': existing.is_admin,
                    'note': 'If you need to reset password, use the forgot password feature or promote user via database.'
                }), 200
